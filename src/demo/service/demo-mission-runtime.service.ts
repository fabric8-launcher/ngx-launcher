  ...
  getMissions(): Observable<Mission[]> {
    let missions = Observable.of([{
    ...
    },
      {
        ...
        'description': 'The Circuit Breaker Mission demonstrates a generic pattern for reporting the failure of a service and then limiting access to the failed service until it becomes available to handle requests. This helps prevent cascading failure in other services that depend on the failed services for functionality.',
        'suggested': false,
        'runtimes': [
          'vert.x',
          'nodejs',
      ...
      },
      {
        'id': 'health-check',
        'name': 'Health Check',
        'description': 'When you deploy an application, its important to know if it is available and if it can start handling incoming requests. Implementing the health check pattern allows you to monitor the health of an application, which includes if an application is available and whether it is able to service requests.',
        'suggested': false,
        'runtimes': [
          'vert.x',
          'spring-boot',
  ...
  getRuntimes(): Observable<Runtime[]> {
    let runtimes = Observable.of([
      ...
      },
      {
        'id': 'wildfly-swarm',
        'name': 'WildFly Swarm',
        'description': 'WildFly Swarm offers an innovative approach to packaging and running Java EE applications by packaging them with just enough of the server runtime to "java -jar" your application.',
        /* tslint:disable */
